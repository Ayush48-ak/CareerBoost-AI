...
export default function MockInterview() {
  ...
  const start = () => {
    ...
    if (!selectedRole) {
      toast.error('Select or type a role first')
      return
    }

    const roleData =
      QUESTIONS[selectedRole] || QUESTIONS['Software Engineer']

    const allQuestions = []

    if (activeTypes.includes('behavioral')) {
      roleData.behavioral.forEach((q) => {
        allQuestions.push({
          q,
          type: 'behavioral',
        })
      ...
      <div className="max-w-2xl space-y-6 animate-slide-up">
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm">
            Practice role-specific behavioral, technical and coding questions
          </p>
        </div>
            ...
            return (
              <div key={index} className="card p-4">
                <div className="flex items-start gap-2 mb-2">
                  <span
                    className={`badge text-xs border ${TYPE_COLORS[type]} flex items-center gap-1`}
                  >
                    <Icon size={9} />
                    {type}
                  </span>
                ...
                {answers[index] ? (
                  <p className="text-sm text-slate-400 pl-2 leading-relaxed">
                    {answers[index]}
                  </p>
                ) : (
                  <p className="text-xs text-slate-600 pl-2 italic">
                    Not answered
                  </p>
                )}
              </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            {role || customRole} · {questions.length} questions
          </p>
        </div>
        ...
        {showHint && (
          <div className="flex items-start gap-2.5 bg-amber-500/5 border border-amber-500/20 rounded-xl p-3">
            ...
            <p className="text-sm text-amber-300/90">
              ...
              {type === 'coding' &&
                'Start with brute force, then optimize. Explain time complexity'}
            </p>
          </div>
        )}
      </div>
